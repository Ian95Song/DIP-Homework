...
cv::Mat_<float> generateNoisyImage(const cv::Mat_<float> &img, dip2::NoiseType noiseType)
{
    ...
    switch (noiseType) {
        case dip2::NOISE_TYPE_1: {
            ...
            cv::randu(tmp1, 0, 1);
            cv::threshold(tmp1, tmp2, noiseLevel, 1, CV_THRESH_BINARY);
            cv::multiply(tmp2,img,tmp2);
            cv::threshold(tmp1, tmp1, 1-noiseLevel, 1, CV_THRESH_BINARY);
            tmp1 *= 255;
            tmp1 = tmp2 + tmp1;
            cv::threshold(tmp1, tmp1, 255, 255, CV_THRESH_TRUNC);
            return tmp1;
        } break;
        case dip2::NOISE_TYPE_2: {
            ...
            tmp1 = img + tmp1;
            cv::threshold(tmp1,tmp1,255,255,CV_THRESH_TRUNC);
            cv::threshold(tmp1,tmp1,0,0,CV_THRESH_TOZERO);
            return tmp1;
        } break;
        default:
