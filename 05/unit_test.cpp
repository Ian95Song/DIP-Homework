...
bool checkMatricesMatchSilent(const cv::Mat_<float> &testMatrix, const cv::Mat_<float> &expectedMatrix, float eps)
{
...
bool checkMatricesMatch(const cv::Mat_<float> &testMatrix, const cv::Mat_<float> &expectedMatrix, const char *indent, float eps, unsigned border=0)
{
    if (!matrixIsFinite(testMatrix)) {
        std::cout << indent << "Matrix contains nan/inf values! Division by zero? Log/sqrt/power of negative number?" << std::endl;
        return false;
    }
    if (testMatrix.cols != expectedMatrix.cols)  {
        std::cout << indent << "Matrix has wrong number of columns! Got " << testMatrix.cols << " but expected " << expectedMatrix.cols << std::endl;
        return false;
    }
    if (testMatrix.rows != expectedMatrix.rows)  {
        std::cout << indent << "Matrix has wrong number of rows! Got " << testMatrix.rows << " but expected " << expectedMatrix.rows << std::endl;
        return false;
    }
    ...
    for (unsigned i = border; i < expectedMatrix.rows-border; i++)
        for (unsigned j = border; j < expectedMatrix.cols-border; j++) {
            if (std::abs(testMatrix(i, j) - expectedMatrix(i, j)) > eps) {
                std::cout << indent << "Given matrix values do not match the expected values! In element " << i <<','<<j<< " got " << testMatrix(i, j) << " but expected " << expectedMatrix(i, j) << std::endl;
                return false;
            }
        }
...
bool test_createGaussianKernel1D()
{
    ...
    gaussian_2_5 << 0.00903789f, 0.0217894f, 0.0447649f, 0.0783688f, 0.116912f, 0.148625f, 0.161003f, 0.148625f, 0.116912f, 0.0783688f, 0.0447649f, 0.0217894f, 0.00903789f;
    cv::Mat_<float> gaussian_3_5(1, 19);
    gaussian_3_5 << 0.00420558f, 0.0084173f, 0.0155263f, 0.0263942f, 0.0413522f, 0.0597084f, 0.0794547f, 0.0974431f, 0.110136f, 0.114724f, 0.110136f, 0.0974431f, 0.0794547f, 0.0597084f, 0.0413522f, 0.0263942f, 0.0155263f, 0.0084173f, 0.00420558f;

    std::cout << "    Testing createGaussianKernel1D with sigma=2.5" << std::endl;
    ok &= checkMatricesMatch(
        dip5::createGaussianKernel1D(2.5f),
        gaussian_2_5,
        "         ",
        1e-3f
    );
    ...
    ok &= checkMatricesMatch(
        dip5::createGaussianKernel1D(3.5f),
        gaussian_3_5,
        "         ",
        1e-3f
    );
...
bool test_separableFilter()
{
    ...
    std::cout << "    Testing different image and kernel sizes to check for crashes" << std::endl;
    {
        ...
        for (auto t : sizes) {
            std::cout << "        testing image size " << t[0]<<'x'<<t[1] << " with kernel sizes " << t[2] << ',' << t[3] << std::endl;
            ...
            if (!matrixIsFinite(result)) {
                std::cout << "             Output matrix contains nan/inf values. Out of bounds access?" << std::endl;
                ok = false;
            }
        }
    }
    std::cout << "    Testing x kernel usage" << std::endl;
    {
        ...
        if (!checkMatricesMatchSilent(img, imgCopy, 1e-8f)) {
            std::cout << "        It seems the input image is being overwritten." << std::endl;
            ok = false;
        }
    ...
    std::cout << "    Testing pattern" << std::endl;
    {
        ...
847f, -461.736f, 804.886f, -21.1197f, -778.29f, 823.041f, 948.465f, -499.838f, -350.992f, 419.604f, 404.404f, 118.096f, 207.777f, 376.341f, -183.099f, -416.743f, 732.976f, 572.83f, -189.594f, -494.774f, -152.759f, -30.4279f, 196.5f, 580.066f, -151.921f, -316.512f, 253.275f, 183.091f, 1111.07f, -27.3755f, -1022.97f, -612.31f, 319.853f, 577.683f, 421.309f, -459.458f, -697.922f, 962.71f, 588.685f, -629.061f, -246.553f, 103.271f, 170.368f, 142.06f, 545.577f, -891.831f, -628.513f, -36.3573f, 217.344f, 195.879f, 28.7014f, -441.331f, 416.148f, 1058.42f, -383.518f, -862.649f, -61.8739f, 180.835f, 84.9961f, 40.3905f;
...
bool test_createFstDevKernel1D()
{
    ...
    gaussianDeriv_2_5 << 0.0282496f, 0.0567557f, 0.0932807f, 0.122478f, 0.12181f, 0.0774257f, -0.0f, -0.0774257f, -0.12181f, -0.122478f, -0.0932807f, -0.0567557f, -0.0282496f;
    cv::Mat_<float> gaussianDeriv_3_5(1, 19);
    gaussianDeriv_3_5 << 0.013903f, 0.0247346f, 0.0399215f, 0.0581705f, 0.075947f, 0.0877278f, 0.0875554f, 0.0715852f, 0.040455f, -0.0f, -0.040455f, -0.0715852f, -0.0875554f, -0.0877278f, -0.075947f, -0.0581705f, -0.0399215f, -0.0247346f, -0.013903f;


    auto scaleLike = [](const char *indent, cv::Mat_<float> &matrix, const cv::Mat_<float> &ref) -> bool {
...
bool test_calculateDirectionalGradients()
{
    ...
    img << 32.0f, 33.2651f, 26.3926f, 28.5712f, 41.9099f, 29.7759f, 26.8128f, 34.4413f, 36.1201f, 22.8396f, 38.1588f, 31.6285f, 42.4473f, 16.3225f, 16.8063f, 27.7021f, 26.0137f, 47.1719f, 34.1924f, 20.9584f, 44.2924f, 25.7091f, 12.2011f, 19.6166f, 21.0718f, 31.494f, 42.8133f, 38.3929f, 29.8967f, 27.832f, 46.6638f, 34.9323f, 42.3362f, 26.8677f, 25.1096f, 40.5968f, 32.3854f, 29.7782f, 30.7645f, 29.0499f, 40.3967f, 35.7961f, 31.1214f, 40.224f, 26.0212f, 27.1307f, 30.2984f, 41.291f, 16.2277f, 37.5835f, 30.2681f, 32.7841f, 34.1016f, 25.3005f, 37.192f, 30.6456f, 45.3197f, 29.7067f, 25.5866f, 37.6771f, 36.7523f, 37.4903f, 28.1094f, 27.3598f, 32.9971f, 24.3329f, 24.335f, 41.3664f, 37.1681f, 34.9297f, 22.6319f, 42.6348f, 42.2201f, 43.4664f, 27.9667f, 23.4228f, 40.8765f, 35.9558f, 24.8094f, 27.6855f, 31.0767f, 24.4108f, 36.1831f, 32.9175f, 32.2764f, 24.2278f, 38.9018f, 42.0546f, 25.904f, 36.4529f, 50.2594f, 52.6911f, 45.3649f, 31.2124f, 41.4535f, 16.1447f, 41.5527f, 33.0023f, 43.4248f, 20.5059f, 32.4029f, 39.4488f, 26.4535f, 19.6118f, 20.9359f, 21.9533f, 23.07f, 32.5872f, 34.832f, 29.02f, 30.7457f, 34.6818f, 35.8299f, 33.9309f, 34.9891f, 32.9152f, 40.0879f, 32.6394f, 14.1186f, 20.9652f;
, 3.58978f, -0.971522f, -2.67839f, -2.20746f, -2.25252f, 2.63633f, -5.95538f, -2.49264f, -1.52806f, 0.549779f, 2.14003f, -0.243018f, 1.46696f, 0.666214f, 0.11588f, 2.83505f, -2.1034f, -0.432755f, -1.45308f, -2.065f, 0.158397f, -1.02656f, -1.34213f, 1.63668f, 4.46386f, 0.981487f, -2.15065f, 1.97577f, 4.38636f, 4.05518f, -1.00003f, -2.73884f, -0.93262f, 1.10851f, -0.321147f, -3.31605f, -1.90044f, -1.73839f, 1.85356f, 2.32043f, 0.536983f, 0.891178f, 3.12731f, 0.752759f, -1.93698f, -1.12623f, 10.0989f, 5.37889f, 0.651634f, -1.68166f, 0.770774f, -0.71188f, -0.813728f, 0.725495f, 3.05334f, -0.961976f, 2.0923f, 2.3893f, -2.59996f, -5.34592f, -4.09501f, -2.91321f, -4.78837f, -3.19749f, -0.180648f, -2.16525f, -8.23406f, -6.78347f, -3.72766f, -0.82774f, 0.506435f, 3.4979f, 1.01518f, -3.37814f, -7.57117f, -3.55205f, -2.11362f, -1.27558f, 2.65255f, 5.50964f, 5.92658f, 6.081f, 5.12668f, -0.729739f, -6.49804f, -5.06749f;

    std::cout << "    Testing with sigma=0.5" << std::endl;
    {
        float scale = getDerivativeScalingFactor(0.5f);
        if (scale < 1e-8f) {
            std::cout << "        Test futile because createFstDevKernel1D produces too small values" << std::endl;
            ok = false;
        }
...
bool test_calculateStructureTensor()
{
    ...
    std::cout << "    Testing different image sizes to check for crashes" << std::endl;
    {
        ...
        for (auto t : sizes) {
            std::cout << "        testing image size " << t[0]<<'x'<<t[1] << " sigma=3.5" << std::endl;
            ...
            if (!matrixIsFinite(A00)) {
                std::cout << "             A00 matrix contains nan/inf values. Out of bounds access?" << std::endl;
                ok = false;
            }
            if (!matrixIsFinite(A01)) {
                std::cout << "             A01 matrix contains nan/inf values. Out of bounds access?" << std::endl;
                ok = false;
            }
            if (!matrixIsFinite(A11)) {
                std::cout << "             A11 matrix contains nan/inf values. Out of bounds access?" << std::endl;
                ok = false;
            }
        }
    }
    ...
    std::cout << "    Testing pattern" << std::endl;
    {
        ...
        gx << -1.60309e-09f, 0.158135f, -0.700928f, -0.428596f, 1.23874f, -0.278008f, -0.648399f, 0.305163f, 0.515015f, -1.14504f, 0.769846f, -0.0464312f, 1.30591f, -1.95968f, -1.89921f, -0.537238f, -0.74829f, 1.89649f, 0.27405f, -1.38021f, 1.53655f, -0.786357f, -2.47486f, -1.54792f, -1.36602f, -0.0632474f, 1.35166f, 0.799109f, -0.262907f, -0.521005f, 1.83298f, 0.36654f, 1.29202f, -0.64154f, -0.861299f, 1.0746f, 0.0481772f, -0.277725f, -0.154436f, -0.368766f, 1.04959f, 0.474517f, -0.109831f, 1.028f, -0.747347f, -0.608659f, -0.2127f, 1.16137f, -1.97154f, 0.697937f, -0.216486f, 0.0980085f, 0.262698f, -0.837436f, 0.648999f, -0.169301f, 1.66496f, -0.286667f, -0.801671f, 0.709641f, 
, 1.00841f, 1.08716f, 1.8116f, 1.05885f, 0.899465f, 1.0546f, 0.15158f, 0.254164f, 0.679718f, 1.06201f, 1.12631f, 0.987843f, 1.39544f, 1.29435f, 0.705865f, 0.40053f, 0.658409f, 0.358761f, 0.392256f, 0.563998f, 0.814811f, 1.32763f, 2.11848f, 2.54756f, 1.76173f, 1.19684f, 1.88196f, 1.30999f, 0.637912f, 0.396619f, 0.612922f, 0.929275f, 1.19555f, 1.74097f, 2.46146f, 2.55119f, 1.58805f, 2.18247f, 1.27254f, 0.464739f, 0.380458f, 0.400396f, 0.541388f, 0.722385f, 0.99866f, 1.06591f, 0.665243f, 0.916429f, 0.68402f, 0.654617f, 0.568404f, 0.405619f, 0.661049f, 0.634219f, 0.285259f, 0.1871f, 0.667149f, 0.352816f, 0.605029f, 1.23686f, 0.909644f, 0.506741f, 0.697545f, 0.543471f, 0.229241f, 0.208469f, 0.962229f, 0.690339f, 1.31587f, 1.96304f, 1.00159f, 0.566314f, 0.92783f, 0.927454f, 0.512973f, 0.276292f, 0.808207f, 1.1285f, 2.86787f, 4.46062f, 2.04174f, 1.1909f, 2.04018f, 1.86565f, 1.04791f, 0.319028f;
...
bool test_calculateFoerstnerWeightIsotropy()
{
    ...
    std::cout << "    Testing different image sizes to check for crashes" << std::endl;
    {
        ...
        for (auto t : sizes) {
            std::cout << "        testing image size " << t[0]<<'x'<<t[1] << std::endl;
            cv::Mat_<float> A00, A01, A11;
            ...
            if (!matrixIsFinite(weight)) {
                std::cout << "             weight matrix contains nan/inf values. Out of bounds access? Div by zero?" << std::endl;
                ok = false;
            }
            if (!matrixIsFinite(isotropy)) {
                std::cout << "             isotropy matrix contains nan/inf values. Out of bounds access? Div by zero?" << std::endl;
                ok = false;
            }
        }
    }
    ...
    std::cout << "    Testing pattern" << std::endl;
    {
        ...
        A01 << 0.112398f, -0.0292913f, -0.152662f, -0.301208f, -0.536195f, -0.237921f, 0.0153954f, 0.0562621f, 0.30871f, 0.849838f, -0.184975f, -0.0915838f, -0.119442f, -0.400112f, -0.487019f, -0.242031f, 0.0760061f, 0.390824f, 0.472772f, 0.929172f, -1.16789f, -0.533666f, -0.493099f, -0.792295f, -0.652415f, -0.559777f, -0.739749f, -0.116642f, 0.113417f, 0.00165269f, -0.312372f, -0.170728f, -0.169365f, -0.493607f, -0.686037f, -0.509298f, -0.404478f, -0.033428f, 0.0285372f, -0.175273f, 0.095514f, 0.00836803f, -0.0765183f, -0.460895f, -0.586372f, -0.119722f, 0.361652f, 0.45518f, 0.314012f, 0.1125f, 0.207564f, 0.11109f, 0.0565398f, -0.0608462f, -0.100004f, 0.417736f, 0.978812f, 0.67929f, 0.232393f, 0.119717f, 0.500515f, 0.612329f, 0.438808f, 0.279513f, 0.228952f, 0.365177f, 0.288249f, 0.11728f, -0.177166f, 0.140048f, 0.216659f, 1.09863f, 0.93988f, 0.445924f, 0.232748f, 0.0462291f, -0.357059f, -0.371468f, -0.206517f, 0.0460247f, -
        std::vector<cv::Vec2i> expectedKeypoints = {{16,4},{29,6},{35,8},{56,8},{75,9},{78,10},{145,10},{85,12},{3,13},{1,16},{13,22},{5,23},{120,36},{97,38},{129,39},{92,43},{110,43},{25,44},{122,46},{131,53},{92,56},{126,57},{101,70},{112,73},{46,78},{51,80},{76,81},{157,81},{121,94},{41,96},{81,96},{157,97},{99,98},{130,98},{149,98},{119,99},{135,101},{151,101},{101,103},{29,104},{43,105},{120,105},{48,106},{129,106},{142,106},{119,109},{6,110},{104,110},{157,110},{55,112},{118,112},{67,113},{81,113},{123,113},{144,113},{13,114},{101,117},{117,118},{87,119},{57,120},{43,121},{125,121},{155,122},{21,124},{29,124},{35,124},{6,125},{45,125},{74,126},{98,126},{84,128},{153,129},{116,130},{134,131},{139,134},{45,135},{118,135},{158,135},};

        cv::Mat img = cv::imdecode(cv::_InputArray((const char *)data_inputImage, data_inputImage_size), 0);
        img.convertTo(img, CV_32FC1);


        auto keypoints = dip5::getFoerstnerInterestPoints(img, 0.5f, 2.0f, 1.5f, 0.5f);

        auto countOutsideBorder = [](const std::vector<cv::Vec2i> &keypoints, int minX, int minY, int maxX, int maxY)->unsigned {
            unsigned count = 0;
            for (auto kp : keypoints) {
                if ((kp[0] > minX) && (kp[0] < maxX) &&
                    (kp[1] > minY) && (kp[1] < maxY))
                    count++;
            }
            return count;
        ...
        for (auto kp : keypoints) {
            if ((kp[0] < 0) || (kp[0] >= img.cols) ||
                (kp[1] < 0) || (kp[1] >= img.rows)) {
                std::cout << "        Detected a keypoint at " << kp[0]<<','<<kp[1] << " which is outside of the image bounds. x and y swapped?" << std::endl;
                ok = false;
                break;
            }
        }
        ...
        int expectedCount = countOutsideBorder(expectedKeypoints, 10, 10, img.cols-10, img.rows-10);
        int count = countOutsideBorder(keypoints, 10, 10, img.cols-10, img.rows-10);

        unsigned maxDifferent = 5;

        if (std::abs(expectedCount - count) > maxDifferent) {
            std::cout << "        Outside a 10px image border " << count << " interest points were detected but " << expectedCount << " were expected." << std::endl;
            ok = false;
        }

        auto closestSqrDistance = [](const std::vector<cv::Vec2i> &keypoints, const cv::Vec2i &query)->unsigned {
            unsigned result = -1;
            for (auto kp : keypoints) {
                int dx = kp[0] - query[0];
                int dy = kp[1] - query[1];
        ...
        if (!keypoints.empty()) {
            ...
            for (auto kp : expectedKeypoints) {
                if ((kp[0] < 10) || (kp[0] > img.cols-10) ||
                    (kp[1] < 10) || (kp[1] > img.rows-10))
                        continue;
            ...
            if (falseNegatives.size() > maxDifferent) {
                std::cout << "        Too many expected keypoints were not detected:" << std::endl;
                ok = false;
                for (auto v : falseNegatives)
                    std::cout << "            Expected a keypoint at " << v.first[0]<<','<<v.first[1] << " but closest detected keypoint is " << v.second << " pixels away." << std::endl;
            }
            ...
            if (falsePositives.size() > maxDifferent) {
                std::cout << "        Too many detected keypoints were not expected:" << std::endl;
                ok = false;
                for (auto v : falsePositives)
                    std::cout << "            Detected a keypoint at " << v.first[0]<<','<<v.first[1] << " but closest expected keypoint is " << v.second << " pixels away." << std::endl;
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    if (!ok) {
        ...
        std::cout << std::endl << "==== Everything seems(!) to be ok. ====" << std::endl;
        return 0;
    }
}
